<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use OpenApi\Annotations as OA;

class NotaryController extends Controller
{
    /**
     * @OA\Get(
     *   path="/api/notaries",
     *   summary="List notaries (paginated)",
     *   tags={"Notaries"},
     *   @OA\Parameter(name="district", in="query", description="Filter by district", @OA\Schema(type="string")),
     *   @OA\Parameter(name="q", in="query", description="Search in professional_address or authorized_area", @OA\Schema(type="string")),
     *   @OA\Parameter(name="page", in="query", description="Page number (default 1)", @OA\Schema(type="integer", minimum=1)),
     *   @OA\Parameter(name="per_page", in="query", description="Items per page (default 20, max 100)", @OA\Schema(type="integer", minimum=1, maximum=100)),
     *   @OA\Response(
     *     response=200,
     *     description="Paginated notaries",
     *     @OA\JsonContent(
     *       type="object",
     *       @OA\Property(property="data", type="array", @OA\Items(
     *         type="object",
     *         @OA\Property(property="id", type="integer"),
     *         @OA\Property(property="sr_no", type="integer"),
     *         @OA\Property(property="professional_address", type="string"),
     *         @OA\Property(property="authorized_area", type="string"),
     *         @OA\Property(property="contact_no", type="string"),
     *         @OA\Property(property="district", type="string")
     *       )),
     *       @OA\Property(property="pagination", type="object",
     *         @OA\Property(property="page", type="integer"),
     *         @OA\Property(property="per_page", type="integer"),
     *         @OA\Property(property="total", type="integer"),
     *         @OA\Property(property="total_pages", type="integer")
     *       )
     *     )
     *   )
     * )
     */
    public function index(Request $request)
    {
        $district = $request->query('district');
        $qText = $request->query('q');
        $page = max(1, (int) $request->query('page', 1));
        $perPage = min(100, max(1, (int) $request->query('per_page', 20)));

        $baseQuery = DB::table('citizen.notaries')
            ->select(['id', 'sr_no', 'professional_address', 'authorized_area', 'contact_no', 'district']);

        if ($district) {
            $baseQuery->where('district', $district);
        }
        if ($qText) {
            $baseQuery->where(function ($sub) use ($qText) {
                $sub->where('professional_address', 'like', '%' . $qText . '%')
                    ->orWhere('authorized_area', 'like', '%' . $qText . '%');
            });
        }

        $total = (clone $baseQuery)->count();
        $data = $baseQuery->orderBy('id')->forPage($page, $perPage)->get();

        return response()->json([
            'data' => $data,
            'pagination' => [
                'page' => $page,
                'per_page' => $perPage,
                'total' => (int) $total,
                'total_pages' => (int) ceil($total / $perPage),
            ],
        ]);
    }
}
